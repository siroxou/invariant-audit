...
import { RULES, runRules, sliceCall, stripped, unknownSuppressions, walkFiles } from "./rules.ts";
import { validate } from "./findings.ts";
import { layering } from "./rules.ts";
...
test("promise-timer: a sleep is not a leak, only a deadline is", async () => {
	// `new Promise(r => setTimeout(r, n))` is the canonical sleep — the timer firing IS the
	// result, so there is nothing to clear. Flagging it was the rule's one false positive,
	// found by running this tool against an unrelated codebase.
	const sleep = await scan("promise-timer-never-cleared", {
		"src/sleep.ts": `await new Promise((r) => setTimeout(r, 4000));\n`,
	});
	assert.equal(sleep.length, 0, "a sleep has no deadline to disarm");

	const deadline = await scan("promise-timer-never-cleared", {
		"src/rpc.ts": `function f() {\nreturn new Promise((res, rej) => {\nsetTimeout(() => rej(new Error("late")), 20);\n});\n}\n`,
	});
	assert.equal(deadline.length, 1, "a rejecting timer is a deadline and must be cleared");
});

test("layering: value imports across a forbidden boundary fire, type imports do not", async () => {
	const root = await mkdtemp(join(tmpdir(), "llm-rules-"));
	await mkdir(join(root, "src/core"), { recursive: true });
	await mkdir(join(root, "src/ui"), { recursive: true });
	await writeFile(join(root, "src/core/a.ts"), 'import { send } from "../ui/server.ts";\n');
	await writeFile(join(root, "src/ui/b.ts"), 'import { load } from "../core/a.ts";\n');

	const rule = layering({ "src/core": ["src/ui"] }, { typeOnly: true });
	const found = await runRules(root, [rule]);
	assert.deepEqual(found.map((f) => f.file), ["src/core/a.ts"], "only the forbidden direction fires");

	await writeFile(join(root, "src/core/a.ts"), 'import type { S } from "../ui/server.ts";\n');
	const typed = await runRules(root, [rule]);
	assert.deepEqual(typed, [], "a type-only import erases at compile time and is not an edge");
});
