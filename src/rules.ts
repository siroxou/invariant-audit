// invariant-audit — the deterministic pass.
//
// Invariant checks over source text. No model, no dependencies, no network.
//
// Every rule here asserts a property a codebase *already holds at every site but
// one or two*. That is what separates an invariant from a style rule: it names
// discipline that already exists and stops it eroding, so it can be enforced
// without asking anyone to change how they write code. A rule that fires on
// healthy code is worse than no rule, because it teaches people to ignore the
// tool — and it takes the useful rules down with it.
//
// ponytail: line/regex analysis over `stripped()` source, no AST. Loading a
// compiler to decide "does this call have an error listener" is not worth it.
// Upgrade to a real parser if a rule ever needs scope or data-flow analysis.

import { readFile, readdir } from "node:fs/promises";
import { join, relative, sep } from "node:path";
import { normalizeEvidence, type Finding, type Severity } from "./findings.ts";

/** Directories never worth scanning. Build output and vendored code are not yours to fix. */
export const SKIP_DIRS = new Set([
	"node_modules", ".git", "dist", "build", "out", "coverage", ".next", ".nuxt",
	".svelte-kit", ".cache", ".turbo", "vendor", "target", "__pycache__",
]);

/** Visit every file under `dir`, passing the absolute path and the path relative to `root`. */
export async function walkFiles(dir: string, root: string, visit: (abs: string, rel: string) => Promise<void>): Promise<void> {
	let entries;
	try {
		entries = await readdir(dir, { withFileTypes: true });
	} catch {
		return; // unreadable directory, or a file path passed in
	}
...

	{
		...
		scan(src, code) {
			...
			for (const p of matches(/new\s+Promise\s*\(/g, src, code)) {
				...
				// A deadline rejects; a sleep resolves. `new Promise(r => setTimeout(r, 1000))` is
				// the canonical sleep and has nothing to clear — the timer firing IS the result.
				// Only the executor's rejector, whatever it is named, marks this as a deadline.
				const rejector = body.match(/^\s*\(?\s*[\w$]+\s*,\s*([\w$]+)\s*\)?\s*=>/)?.[1];
...
/**
 * A layering rule for a project's own architecture. Not in RULES, because only the
 * project knows its layers.
 *
 *   layering({ "src/core": ["src/ui", "src/electron"] })
 *
 * reads as: nothing under src/core may import from src/ui or src/electron. Add
 * `typeOnly: true` to permit `import type`, which erases at compile time and so is
 * not a real edge.
 */
export function layering(
	...
	return {
		...
		severity: "medium",
		message: "Layering violation — this module imports from a layer it is meant to know nothing about.",
		include: /\.(ts|tsx|mts|cts|js|jsx|mjs|cjs)$/,
		scan(src, code, path) {
			const layer = Object.keys(forbidden).find((from) => path.startsWith(from.replace(/\/?$/, "/")));
			if (!layer) return [];
			const out: Hit[] = [];
			for (const m of matches(/^\s*import\s[^;]*?from\s*["'][^"']+["']/gm, src, code)) {
				const line = lineText(src, m.line);
				if (opts.typeOnly && /^\s*import\s+type\b/.test(line)) continue;
				const spec = line.match(/from\s*["']([^"']+)["']/)?.[1] ?? "";
				// Resolve a relative specifier against this file so it can be compared to a layer.
				const resolved = spec.startsWith(".")
					? [...path.split("/").slice(0, -1), ...spec.split("/")]
							.reduce<string[]>((acc, part) => (part === ".." ? (acc.pop(), acc) : part === "." || part === "" ? acc : [...acc, part]), [])
							.join("/")
					: spec;
				if (forbidden[layer]!.some((to) => resolved.startsWith(to.replace(/\/?$/, "/")) || resolved === to)) {
...
export function unknownSuppressions(src: string, rules: Rule[] = RULES): string[] {
	const known = new Set([...rules.map((r) => r.id), "layering-violation"]);
	return [...src.matchAll(/audit-ok\(([a-z0-9-]+)\)/g)].map((m) => m[1]!).filter((id) => !known.has(id));
}
